  ...
  botonListar.addEventListener("click", async () => {
    ...
    try {
      const resultado = await fetch(
        "http://localhost:3000/views/Read/Mensajes/listarTodo"
      );
      ...
      datos.forEach((item) => {
        ...
        dniCell.innerText = item.correo;
        row.appendChild(dniCell);

        let nameCell = document.createElement("td");
        nameCell.innerText = item.remitente;
        row.appendChild(nameCell);

        let lnameCell = document.createElement("td");
        lnameCell.innerText = item.telefono;
        row.appendChild(lnameCell);

        let birthdayCell = document.createElement("td");
        birthdayCell.innerText = item.asunto;
        row.appendChild(birthdayCell);

        let genderCell = document.createElement("td");
        genderCell.innerText = item.mensaje;
      ...
      botonEliminar.forEach((element) => {
        element.addEventListener("click", async (event) => {
          if (event.target.id === "delete") {
            ...
            await fetch(
              "http://localhost:3000/views/Read/Mensajes/eliminar/" + id,
              {
                method: "DELETE",
              }
            ).then((response) => {
